	...
	public sealed class JSONTokener
	{
		   ...
		   // private string tag = "snowfish";
		    ...
		    public JSONTokener(string ins) {
		     ...
		     //   }
		        this.ins = ins;
 		     	//APaymentHelperDemo.toAndroidLog(tag, "JSONTokener ins:"+ins);
		    }
...
//			    APaymentHelperDemo.toAndroidLog(tag, "readEscapeCharacter escaped:"+escaped);
		        switch (escaped) {
		            case 'u':
		                if (pos + 4 > ins.Length) {
		                   // throw syntaxError("Unterminated escape sequence");
                         //   APaymentHelperDemo.toAndroidLog(tag, "readEscapeCharacter Unterminated escape sequence");
                            return ' ';
		                }
