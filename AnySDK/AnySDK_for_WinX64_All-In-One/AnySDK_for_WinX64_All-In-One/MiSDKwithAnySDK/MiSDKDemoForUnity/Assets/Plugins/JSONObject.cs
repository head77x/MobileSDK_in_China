...
public class JSONObject : Nullable
{
    ...
    public JSONObject() { }
    public JSONObject(string str, bool clear = false)
            ...
            /*
                        if (clear)
                        {
                            ...
                            for (int i = 0; i < strchr.Length; i++)
                            {
                                ...
                                if (inside == 0)
                                {
                                    if (strchr[i] == '\n' || strchr[i] == '\t' || strchr[i] == '\r' || strchr[i] == '\\' || strchr[i] == ' ')
                                        continue;
                                }

                                tempstr.Add(strchr[i]);
                            }

                            str = new string(tempstr.ToArray());
            ...
            if (str.Length > 0)
            {
                ...
                else
                {
                    ...
                    catch (System.FormatException)
                    {
                        ...
                        {
                            if (!openquote && ( str[i] == '\\' || str[i] == '\t' || str[i] == '\n' || str[i] == '\r' || str[i] == ' ' ) )
                            {
...
//                                Debug.Log(str);
                            ...
                            if (depth == 0 && !openquote)
                            {
                                if (str[i] == ':' && !inProp)
                                {
                                    ...
                                    try
                                    {
//                                        Debug.Log(str.Substring(token_tmp + 2, i - token_tmp - 3));
    ...
    public int GetValue(JSONObject val)
    {
	    int result = 0;
    ...
    public float GetValueFloat(JSONObject val)
    {
	    float result = 0.0f;
    ...
    public string GetString(JSONObject val)
    {
        string result = "";
        if (val == null)
            return result;
    ...
    public System.UInt64 GetValue64( JSONObject val )
    {
	    ...
	    try
	    {
		    result = System.UInt64.Parse( val.str );
	    }
	    catch
	    {
		    result = 0;
